...
public class PromoWebController {
    ...
    public ModelAndView login(@ModelAttribute("loginAccount") Account account) {
        ...
        String targetView = accountService.authenticateUser(account);
        ...
        if ("site.home".equalsIgnoreCase(targetView)) {
            ...
            List<Location>listGeo=new ArrayList<>();
            Location location = new Location();
            location.setLat("47.7606269");
            location.setLng("-122.1713075");
            listGeo.add(location);
            modelAndView.addObject("listGeo",listGeo);
        }
        return modelAndView;
